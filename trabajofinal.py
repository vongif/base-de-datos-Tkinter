...
def limpiar_registro():
    valor_cuenta.set("0"), valor_reparto.set("0"), valor_cliente.set(
        "0"
    ), valor_razon.set(""), valor_direccion.set(""), valor_localidad.set("")
...
def funcion_buscar(tree):
    ...
    for element in records:
        tree.delete(element)
    sql = f"SELECT * FROM clientes WHERE cuenta LIKE '%{entrybusqueda.get()}%' OR reparto LIKE '%{entrybusqueda.get()}%' OR numero_de_cliente LIKE '%{entrybusqueda.get()}%' OR razonsocial LIKE '%{entrybusqueda.get()}%' OR direccion LIKE '%{entrybusqueda.get()}%' OR localidad LIKE '%{entrybusqueda.get()}%' "
    con = base()
    cursor = con.cursor()
    datos = cursor.execute(sql)
    resultado = datos.fetchall()
...
boton_buscar.grid(row=9, column=0, padx=15, pady=8, ipady=3, ipadx=3, sticky=W)
